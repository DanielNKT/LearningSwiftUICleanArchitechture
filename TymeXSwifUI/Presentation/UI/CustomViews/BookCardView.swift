...
struct BookCardView: View {
    ...
    func TopCardView() -> some View {
        VStack(spacing: 15) {
            ...
            /// Book Main Contents
            Image(uiImage: UIImage(named: book.thumbnail) ?? UIImage(systemName: "questionmark.circle")!)
                .resizable()
        ...
        .frame(maxWidth: .infinity)
        .background {
            Rectangle().fill(book.color.gradient)
        }
    ...
    func FixedHeaderView() -> some View {
        HStack(spacing: 10) {
            ...
            .opacity(scrollProperties.offsetY > 20 ? 1 : 0)
            .disabled(!(scrollProperties.offsetY > 20))
...
#Preview {
    GeometryReader { geo in
        BookCardView(book: books[0], size: geo.size) { _ in }
            .padding(.horizontal, 15)
    }
